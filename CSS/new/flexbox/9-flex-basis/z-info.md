...
3. flex items grow/shrink as per space in container
    ...
    => minimum width would be content size
...
## others
>> when dealing with flex items prefer flex-basis over width
>> do not use any min-width/max-width etc when dealing with flex-items